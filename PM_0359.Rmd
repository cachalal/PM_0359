...
#Graphe de chaque KC
ggplot(data_set_0359, aes(x = TIME, y = LOG10_CFU, group = xCMI, color = factor(xCMI))) +
  geom_line() +
  geom_point(shape = 18) +
...
filtered_data <- data_set_0359 %>%
  filter(ID == 2, MOLECULE == "SMX" & ratio == 0)

filtered_data_2 <- data_set_0359 %>%
  filter(ID == 15, MOLECULE == "SMX" & ratio == "1_19")

filtered_data_3 <- data_set_0359 %>%
  filter (ID == 4, MOLECULE == "SMX" & ratio == "1_999")
...
library(writexl)
write_xlsx(tableau_LR_pivot_moyenne, "LR_0359.xlsx")
```

#Equation 1 : Croissance moyenne de l'inoculum
```{r}
#inoculum moyen obtenu apres exposition à ATB pendant 24h (ou 8h)
Ix <- ifelse(areas$ID %in% c(3), 
             areas$area_under_curve / 8, 
             ifelse(areas$ID == 29 & areas$xCMI %in% c(4, 8, 16), 
                    areas$area_under_curve / 8, 
                    ifelse(areas$ID == 35 & areas$xCMI %in% c(4, 16), 
                           areas$area_under_curve / 8, 
...
valeurs_inoculum <- data_set_0359 %>%
  filter(TIME == 0.00) %>%
...
SMX_1_19<- tableau_delta_i_x %>%
  filter(MOLECULE == "SMX" & ratio == "1_19")

SDMX_1_19<- tableau_delta_i_x %>%
  filter(MOLECULE == "SDMX" & ratio == "1_19")

SDZ_1_19<- tableau_delta_i_x %>%
  filter(MOLECULE == "SDZ" & ratio == "1_19")

#1_99
SMX_1_99<- tableau_delta_i_x %>%
  filter(MOLECULE == "SMX" & ratio == "1_99")

SDMX_1_99<- tableau_delta_i_x %>%
  filter(MOLECULE == "SDMX" & ratio == "1_99")

SDZ_1_99<- tableau_delta_i_x %>%
  filter(MOLECULE == "SDZ" & ratio == "1_99")

#1_999
SMX_1_999<- tableau_delta_i_x %>%
  filter(MOLECULE == "SMX" & ratio == "1_999")

SDMX_1_999<- tableau_delta_i_x %>%
  filter(MOLECULE == "SDMX" & ratio == "1_999")

SDZ_1_999<- tableau_delta_i_x %>%
  filter(MOLECULE == "SDZ" & ratio == "1_999")
...
#1_19
smx_1_19 <- nlsLM(
 delta_i_x ~ delta_i_max - ((delta_i_max - delta_i_min) * ((CONC_TMP / EC3LOG)^gam) / (((-3 - delta_i_min) / delta_i_max + 3) + (CONC_TMP / EC4LOG)^gam)),
  data = SMX_1_19,
...
sdmx_1_19 <- nlsLM(
 delta_i_x ~ delta_i_max - ((delta_i_max - delta_i_min) * ((CONC_TMP / EC4LOG)^gam) / (((-4 - delta_i_min) / delta_i_max + 4) + (CONC_TMP / EC4LOG)^gam)),
  data = SDMX_1_19,
  start = start_values_2,
  control = nls.lm.control(maxiter = 200))
